...
vignette: >
  ...
  %\VignetteDepends{xcms,msdata,BiocStyle,faahKO}
...
- `SimilarRtimeParam`: perform a *crude* grouping based on similar retention
  time.
- `EicCorrelationParam`: perform a feature grouping based on correlation of
  EICs.
- `AbundanceCorrelationParam`: perform a feature grouping based on correlation
...
`AbundanceCorrelationParam` object. This approach performs a pairwise
...
Below we use the `groupFeatures` with the `AbundanceCorrelationParam` to group
...
the correlation analysis should be evaluated from data set to data set. Note
also that by default the correlation is performed on log2 transformed signals.

```{r abundance-correlation}
xdata <- groupFeatures(xdata, AbundanceCorrelationParam(threshold = 0.7,
                                                        filled = TRUE))
table(featureGroups(xdata))
```
...
evaluate this for one specific feature group `"FG.025"` by plotting their
pairwise correlation.

```{r abundance-correlation-fg025, fig.width = 8, fig.height = 8, fig.cap = "Pairwise correlation plot for all features initially grouped into the feature group FG.025."}
fts <- grep("FG.025", featureGroups(xdata))
pairs(t(fvals[fts, ]), gap = 0.1, main = "FG.025")
```

Indeed, correlations can be seen only between some of the features in this
retention time feature group, e.g. between *FT034* and *FG036*. Note however
that this abundance correlation suffers from relatively few samples (8 in
total), and a relatively small variance in abundances across these samples.
...
```{r}
fts <- grep("FG.007.02", featureGroups(xdata))
...
```{r example-1-eic, fig.width = 8, fig.height = 6, fig.cap = "EICs of features from feature group FG.007.02, colors label different feature sub-groups. Shown are the actual intensities (left) and intensities normalized to 1 (right)."}
...
```{r}
fts <- grep("FG.030.01", featureGroups(xdata))
eics <- featureChromatograms(xdata, features = fts,
                             filled = TRUE, n = 1)
```
...
```{r example-2-eic, fig.width = 8, fig.height = 6, fig.cap = "EICs of features from feature group FG.030.01, colors label different feature sub-groups. Shown are the actual intensities (left) and intensities normalized to 1 (right)."}
cols <- c("#ff000080", "#00ff0080")
names(cols) <- unique(featureGroups(xdata)[fts])
...
Based on the EIC correlation, the initial feature group *FG.030.01* was grouped
into two separate sub-groups, each consisting of two features, one with a single
peak and one with a double peak.
